Данный инструмент предназначен для сбора и вывода информации по заданному параметру [`parameter`], содержащегося в источнике. 
В качестве `источника` данных используются CSV-файлы. В качестве заданного параметра - столбец в этих файлах. Информация выводится 
по всем странам, которые как значения столбца `country` содержатся в каждой строке. Информация выводится в стандартный поток вывода 
в табличной форме. 

К данным столбца применяется математическая операция и результат группируется по каждой стране. 
...
этом случае пропускать иные символы, которые следуют перед его названием.

>> Обязательными столбцами в каждом CSV-файле являются `country` и [`parameter`]. Где `parameter` — это имя столбца, к данным которого будет 
применена математическая операция `function`. Значение по умолчанию для `parameter` — `gdp`. Если вы хотите установить другое значение, 
убедитесь, что желаемое значение `parameter` существует как название столбца в файле, который вы передаёте команде. Значение по умолчанию 
