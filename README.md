...
содержащегося в `источнике`, путем применения математической операции к данным, собранным для каждой страны. В качестве `источника` данных 
...
>> Названия столбцов должны начинаться только с символов латинского алфавита или арабских цифр. Двойные кавычки допускаются. При этом, 
двойные кавычки в задании параметра `parameter` не участвуют. В остальном, следует учитывать иное при установке параметра `parameter` и в 
этом случае пропускать иные символы.
...
Итак, предположим, что вас интересует вычисление максимального уровня безработицы за весь рассматриваемый период времени, отражённый в этом 
CSV-файле. Тогда, команда для этой цели будет выглядеть следующим образом:
...
То вы получите несопоставимые результаты со  значениями NaN:
